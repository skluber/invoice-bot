    ...
    async (ctx) => {
        ...
        async (ctx) => {
                ...
                if (clientes.length === 1) {
                    ...
                    await ctx.reply(`✅ Cliente seleccionado: *${clientes[0].nombre_clinica}* (${clientes[0].direccion.split(',')[0]})`, {
                        parse_mode: 'Markdown'
                    });
                ...
                const botones = clientes.map(c => {
                    ...
                    return [{
                        text: `${c.nombre_clinica} (${calleCorta})`,
                        callback_data: `cli_${c.id}`
                    }];
                });
...
async function mostrarMenuConceptos(ctx) {
    ...
    await ctx.reply(`📑 *MENÚ GENERAL FACTURA* \n(Líneas añadidas actualmente: ${numElementos})\n\nSelecciona qué elemento deseas incorporar:`, {
        ...
        reply_markup: {
            inline_keyboard: [
                [{
                    text: '🔧  Añadir Reparación',
                    callback_data: 'menu_reparacion'
                }],
                [{
                    text: '🔩  Añadir Recambio',
                    callback_data: 'menu_recambio'
                }],
                [{
                    text: '⏱️  Añadir Mano de Obra',
                    callback_data: 'menu_mano_obra'
                }],
                [{
                    text: '🚗  Añadir Desplazamiento',
                    callback_data: 'menu_desplazamiento'
                }],
                [{
                    text: '🏁  Finalizar factura',
                    callback_data: 'menu_finalizar'
                }],
                [{
                    text: '❌  Descartar factura',
                    callback_data: 'menu_descartar'
                }]
            ]
        }
    });
...
async function mostrarResumenRevision(ctx) {
    ...
    await ctx.reply(mensaje, {
        ...
        reply_markup: {
            inline_keyboard: [
                [{
                    text: '✅ Todo perfecto, emitir PDF',
                    callback_data: 'factura_confirmar'
                }],
                [{
                    text: '❌ Eliminar un elemento de la lista',
                    callback_data: 'factura_eliminar'
                }],
                [{
                    text: '➕ Volver al menú para añadir más',
                    callback_data: 'factura_volver'
                }],
                [{
                    text: '🚨 Cancelar y descartar Factura',
                    callback_data: 'factura_descartar'
                }]
            ]
        }
    });
...
async function generarFacturaClonada(ctx) {
    ...
    const pdfBuffer = await new Promise((resolve, reject) => {
        ...
        doc.text('CANT.', 350, y + 6, {
            width: 40,
            align: 'right'
        });
        doc.text('PRECIO', 400, y + 6, {
            width: 60,
            align: 'right'
        });
        ...
        conceptos.forEach(c => {
            ...
            doc.text(c.concepto, 120, y, {
                width: 220
            });
            doc.text(c.cantidad.toString(), 350, y, {
                width: 40,
                align: 'right'
            });
            doc.text(c.precio_unitario.toFixed(2).replace('.', ','), 400, y, {
                width: 60,
                align: 'right'
            });
            doc.text(c.total_linea.toFixed(2).replace('.', ','), 470, y, {
                width: 70,
                align: 'right'
            });

            // Calculamos altura para saltos de línea dinámicos si la descripción es larga
            const height = doc.heightOfString(c.concepto, {
                width: 220
            });
        ...
        y += 10;
        doc.fontSize(9).text('BASE IMPONIBLE', 400, y);
        doc.text(subtotal.toFixed(2) + ' €', 500, y);
        y += 15;
        doc.text('IVA 21%', 400, y);
    ...
    await ctx.replyWithDocument({
        source: pdfBuffer,
        filename: `Factura_${numFactura.replace('/', '_')}.pdf`
    });
}
// INICIALIZACIÓN
...
const consultarPendientesFuncion = async (ctx) => {
    ...
    await ctx.reply('⚠️ *FACTURAS PENDIENTES DE PAGO* ⚠️', {
        parse_mode: 'Markdown'
    });
    ...
    for (const [nombreClinica, info] of Object.entries(clinicasPendientes)) {
        ...
        info.facturas.forEach(fact => {
            ...
            botonesFila.push([{
                text: `✅ Cobrada Fct: ${fact.numero}`,
                callback_data: `pagar_${fact.id}_${info.clienteId}`
            }]);
        });
        ...
        await ctx.reply(respuesta, {
            parse_mode: 'Markdown',
            reply_markup: {
                inline_keyboard: botonesFila
            }
        });
    }

    await ctx.reply(`--- \n📉 *Total general pendiente:* _${sumaTotalGeneral.toFixed(2).replace('.', ',')} €_`, {
        parse_mode: 'Markdown'
    });
};
...
bot.action(/^pagar_(\d+)_(\d+)$/, async (ctx) => {
    ...
    if (updateError) {
        ...
        return ctx.answerCbQuery('❌ No se pudo actualizar en la base de datos.', {
            show_alert: true
        });
    }
    ...
    let nuevosBotones = facturasRestantes.map(fact => {
        return [{
            text: `✅ Cobrada Fct: ${fact.numero_factura}`,
            callback_data: `pagar_${fact.id}_${clienteId}`
        }];
    });
